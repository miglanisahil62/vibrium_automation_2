...
Runs at ~07:45 IST (after the 07:35 enrollment poller finishes) and sends
a segment-by-segment funnel of today's cohort to the collections team.

Funnel layers:
  1. Candidates fetched from collection_view (ageing=1) → from dated CSV
...
Cron (07:45 IST = 02:15 UTC):
    15 2 * * *  /home/ubuntu/vibrium-workflow/run.sh enrollment_funnel_emailer
"""
...
def _build_html(f: dict) -> str:
    ...
    if csv_c is not None:
        table_rows += _row("Fetched from collection_view (ageing=1)", csv_c, bold=True)
    else:
        table_rows += _row("Fetched from collection_view (ageing=1)", None, bold=True)
